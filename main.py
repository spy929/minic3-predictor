...
# ==================== Page 5: Biomarker Analysis ====================
    ...
    with tab1:
        col1, col2 = st.columns(2)
        with col1:
            fig = px.box(df, x='PDL1', y='Response', title='PD-L1 and Response', points='all')
            st.plotly_chart(fig, use_container_width=True)
        with col2:
    ...
    with tab2:
        col1, col2 = st.columns(2)
        with col1:
            fig = px.scatter(df, x='TMB', y='Response', color='PDL1', title='TMB and Response')
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            df['TMB_Group'] = pd.cut(df['TMB'], bins=[0, 5, 10, 20, 50], labels=['Low', 'Med', 'High', 'Very High'])
            tmb_response = df.groupby('TMB_Group')['Response'].mean() * 100
            st.bar_chart(tmb_response)

# ==================== Footer ====================
st.markdown("---")
st.markdown(f"<div style='text-align: center; color: gray;'>© 2024 MINIC3 Prediction System | Updated: {datetime.now().strftime('%Y-%m-%d')}</div>", unsafe_allow_html=True)
